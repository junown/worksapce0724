...
public class UpdatePwdController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int updateMember = new MemberService().updateMemberPwd(userId, updatePwd);
		if(updateMember < 0) { //업데이트 실패
			request.setAttribute("errorMsg", "비밀번호 수정에 실패하였습니다");
			request.getRequestDispatcher("views/common/error.jsp").forward(request, response);
		} else {
			Member updatedMember = new MemberService().selectUpdatedMember(userId);
