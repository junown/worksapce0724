...
public class MemberController {
	...
	// 회원이름으로 키워드 검색
	public void keywordMember(String keyword) {
		ArrayList<Member> list = ms.keywordMember(keyword);

		if (list.isEmpty()) {
			new MemberMenu().displaySuccess("성공적으로 회원정보 키워드를 가져왔습니다.");
		} else {
			new MemberMenu().displayFail("회원정보 키워드를 가져오지못하였습니다");
		}
	}
}
