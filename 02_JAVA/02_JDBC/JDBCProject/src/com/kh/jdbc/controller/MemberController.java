...
import com.kh.jdbc.modle.vo.Member;
...
public class MemberController {
	...
	public void updateMember(String userId, String email, String phone, String address, String hobby) {
		...
		if(result > 0) {
			new MemberMenu().displaySuccess("성공적으로 회원정보를 수정실패");
		} else {
			new MemberMenu().displayFail("회원정보를 삭제하는데 실패했다");
		}
		md.updateMember(m);
	}
	...
	public void deleteMember(String userId, String userPwd) {
		...
		if(result > 0) {
			new MemberMenu().displaySuccess("성공적으로 회원정보를 삭제");
		} else {
			new MemberMenu().displayFail("회원정보를 삭제하는데 실패했다");
		}
		md.updateMember(m);
