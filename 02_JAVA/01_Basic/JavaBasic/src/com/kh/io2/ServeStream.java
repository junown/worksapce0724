...
public class ServeStream {
	...
	public void fileSaveWithStream() {
		...
		try {		
			...
			OutputStreamWriter osw = new OutputStreamWriter(fo, "URF-8");
	...
	public void fileRead() {
		...
		try(BufferedReader br = new BufferedReader(new FileReader("1_buffer.txt"))) {
			...
			System.out.println(br.readLine());
			System.out.println(br.readLine());
			System.out.println(br.readLine()); //파일에 모든 내용을 읽어온 후엔 null이 반환
			*/
			String text = null;
	...
	public void ObjectSave() {
		...
		try (ObjectOutputStream oss = new ObjectOutputStream(new FileOutputStream("product1.dat"));){
			oss.writeObject(p1); //직렬화 후 파일에 객체를 저장.
			oss.writeObject(p2);
			oss.writeObject(p3);
		} catch (IOException e) {
			e.printStackTrace();
