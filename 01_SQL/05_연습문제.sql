...
((SALARY + (SALARY*NVL(BONUS, 0) )) * 12) - ((SALARY * 12) * 0.03) AS 총수령액
FROM EMPLOYEE;
...
SELECT EMP_NAME, SALARY, ((SALARY + (SALARY*NVL(BONUS, 0) )) * 12) - ((SALARY * 0.03) * 12), HIRE_DATE
FROM EMPLOYEE
WHERE ((SALARY + (SALARY*NVL(BONUS, 0) )) * 12) - ((SALARY * 0.03) * 12) >= 50000000;
...
-- S22. EMPLOYEE 테이블에서 부서코드가 D5, D6, D9인 사원만 조회하되 D5면 총무부, D6면 기획부, D9면 영업부로 처리 (단, 부서코드 오름차순으로 정렬)
