...
/*
        *서브쿼리 구문
        서브쿼리를 수행한 결과값이 몇행, 몇열로 나오는냐에 따라서 분류
...
FROM EMPLOYEE
WHERE SALARY = (SELECT MIN(SALALY) FROM EMPLOYEE);
...
/*
        ...
        비교대상 > ANY(값~) : 여러개의 결과값 중 한개자도 비교연산을 만족하는가?
...
FROM EMPLOYEE
WHERE EMP_NAME = '유재식') OR 
JOB_CODE = (SELECT JOB_CODE
FROM EMPLOYEE
WHERE EMP_NAME = '윤은해');

--IN절을 통해 다중해 서브쿼리로 처리할 수 있음
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
WHERE (JOB_CODE = 'J1' AND SALARY = 8000000)
        ...
        OR (JOB_CODE = 'J3' AND SALARY = 1550000)
        OR (JOB_CODE = 'J4' AND SALARY = 3400000)
        OR (JOB_CODE = 'J5' AND SALARY = 1380000)
...
-- 각 부서별 최고급여를 받는 사원들의 사번, 사원명, 수버코드, 급여
...
-- 가장 최근에 입사원 사원 5명 조회(사원명, 급여, 입사일)
SELECT *
