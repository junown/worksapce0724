/*
    ...
    (무작정 다 조회해 오는 것이 아니라 각 테이블당 연결고리(외래키)를 통해 데이터를 매칭시켜줘야한다)
...
/*
        ...
        단, 반드시 LEFT/RIGHT을 지정해야된다.(어떤 테이블을 기준으로 할것인가)
*/
-- 외부조인과 비교할만한 내부조인
-- 사원명, 부서명, 급여, 연봉
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
  ...
  --1) LEFT (OUTER) JOIN : 두 테이블 중 왼편에 기술된 테이블을 기준으로 JOIN을 한다
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY * 12
FROM EMPLOYEE
...
/*
        ...
        ON이니 USONG조건을 지정하지 않아도 두 테이블간의 공통된 컬럼을 기반으로 조인
