/*
    <DCL : 데이터 제어문>
    계정에 시스템권한 또는 객체접근 권환을 부여하거나 회수하는 구문
...
/*
    <TCL : 트랜잭션 제어문>
    트랜잭션은 하나이상의 DML(INSERT, UPDATE, DELETE)문을 묶어서 하나의 논리적 잡업 단위로 처리하는 개념
...
/*
    ...
    C(Consistency) 일관성 : 트랜잭션 수행 전후에 DB상태가 항상 유효한 상태(제약조건 충족)이여한다.
