/*
    ...
    일반 SQL의 단점을 보완해서 변수의 정의, 조건(IF), 반복(FOR, WHILE)등을 지원함. 다수의 SQL문을 한번에 실행할 수 있음.
    PL/SQL은 블록구조를 사용함.
...
----------------------------------------------------------------------------------------
/*
    1_2) 래퍼런스 타입 변수 : 어떤 테이블의 어떤 컬럼의 데이터타입을 참조해서 사용할지 정하는 방식
*/
...
------------------------------------------------------------------------
/*
    래퍼런스타입 변수로 EID, ENAME, JOB_CODE, SAL, DTITLE을 선언하고
    각 자료형 EMPLOYEE(EMP_ID, EMP_NAME, JOB_CODE, SALARY), DEPARTMENT(DEPT_TITLE)을 참조하도록
    사용자가 입력한 사번의 사번, 사원명, 직급코드, 급여, 부서명 조회 후 각 변수에 담어서 출력
*/
...
BEGIN
    ...
    INTO EID, ENAME, JOB_CODE, SAL, DTITLE
    FROM EMPLOYEE
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('E.EMP_ID : ' || E.EMP_ID);
    DBMS_OUTPUT.PUT_LINE('E.EMP_NAME : ' || E.EMP_NAME);
    DBMS_OUTPUT.PUT_LINE('E.BONUS : ' || E.BONUS);
END;
/
...
-- 입력받은 사번에 해당하는 사원의 사원, 사원명, 직급코드, 보너스 조회
--+ 보너스를 받지않는 사원은 보너스를 지급하지 않는 사원입니다.
...
BEGIN
    ...
    INTO EID, ENAME, JOB_CODE, SAL, BONUS
    FROM EMPLOYEE
    WHERE EMP_ID = &사번;
...
--    WHEN ZERO_DIVIDE THEN DBMS_OUTPUT.PUT_LINE('나누기 연산자는 0으로 나눌 수 없습니다');
    WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('예외가 발생 하였습니다.');
END;
/
