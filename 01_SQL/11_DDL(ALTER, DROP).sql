/*
    ...
    <ALTER>
    객체를 변경하는 구문
    기존에 생성된 객체(테이블, 컬럼, 제약조건 등)의 구조를 변경(수정, 추가, 제거, 이름변경 등등)을 할 때 사용.
...
--LNAME컬럼 추가 -> 기본값 '한국' 
ALTER TABLE DEPARTMENT ADD LNAME VARCHAR(20) DEFAULT '한국';
SELECT * FROM DEPARTMENT;

--1_2) 컬럼 수정
-- 테이터타입 수정 : MONDIFY 컬럼명 변경하고자하는 데이터타입
...
-- 만약 지우고자한다면
-- 1. 자식테이블 데이터 또는 테이블 자체를 먼저 전부 삭제
...
ALTER TABLE DEPT_COPY RENAME CONSTRAINT DEPT_PK TO DEPT_ID_NN;

--3_3) 테이블명도 변경 : RENAME TO 변경할 테이블명
ALTER TABLE DEPT_COPY RENAME TO DEPT_TSET;
